...
// This comment is set here to trigger full shader cache rebuild, as we need it to update shader targets on Vulkan platform

// make sure the define scaner finds it
#if defined(_AP_PC)
#endif

#if defined(_API_DX11) || defined(_API_VULKAN)
   #if defined(_AP_PC) || defined(_AP_DURANGO) || defined(_API_VULKAN)
      ...
      #define ISOLATE_VALUE(value)                    value
      #define PRECISE_VARIABLE(type, name)            precise type name
      ...
      #define RWTexture2D_UINT4        RWTexture2D<uint4>
      ...
      #define RWTexture3D_FLOAT4       RWTexture3D<float4>
      ...
      #define _SAMPLE_ARRAY_GRAD(tex, samp, uv, ddx, ddy)         _SAMPLE_GRAD(tex, samp, uv, ddx, ddy)
      ...
      #define _SAMPLE_GATHER_RED(tex, samp, uv)                   tex.GatherRed(samp, uv)
      #define _SAMPLE_GATHER_GREEN(tex, samp, uv)                 tex.GatherGreen(samp, uv)
      #define _SAMPLE_GATHER_BLUE(tex, samp, uv)                  tex.GatherBlue(samp, uv)
      #define _SAMPLE_GATHER_ALPHA(tex, samp, uv)                 tex.GatherAlpha(samp, uv)
      #define _SAMPLE_GATHER_OFFS(tex, samp, uv, offs)            tex.Gather(samp, uv, offs)
      
      #define _LOAD_2D_FLOAT(tex, xy)                             tex.Load(int3(xy, 0))
      ...
      bool any1b(bool  v) { return any(v); }
      bool any2b(bool2 v) { return any(v); }
      bool any3b(bool3 v) { return any(v); }
      bool any4b(bool4 v) { return any(v); }
      ...
      bool all1b(bool  v) { return all(v); }
      bool all2b(bool2 v) { return all(v); }
      bool all3b(bool3 v) { return all(v); }
      bool all4b(bool4 v) { return all(v); }
   ...
   //   #pragma warning(disable : 4200)
      ...
      #define HLSL_ATTRIB_EARLYDEPTHSTENCIL           [earlydepthstencil]
      #define ISOLATE_VALUE(value)                    __invariant(value)
      #define PRECISE_VARIABLE(type, name)            type name
      ...
      #define RWTexture2D_UINT4        RW_Texture2D<uint4>
      ...
      #define RWTexture3D_FLOAT4       RW_Texture3D<float4>
      ...
      bool any1b(bool  v) { return v; }
      bool any2b(bool2 v) { return v[0] || v[1]; }
      bool any3b(bool3 v) { return v[0] || v[1] || v[2]; }
      bool any4b(bool4 v) { return v[0] || v[1] || v[2] || v[3]; }
      ...
      bool all1b(bool  v) { return v; }
      bool all2b(bool2 v) { return v[0] && v[1]; }
      bool all3b(bool3 v) { return v[0] && v[1] && v[2]; }
      bool all4b(bool4 v) { return v[0] && v[1] && v[2] && v[3]; }
   ...
   #define RWTexture2D_UINT4        uimage2D
   ...
   #define RWTexture3D_FLOAT4       image3D
   ...
   #define _SAMPLE_ARRAY_GRAD(tex, samp, uv, ddx, ddy)         textureGrad(sampler2DArray(COMB(tex, samp)), uv, ddx, ddy)
   ...
   #define _SAMPLE_GATHER_RED(tex, samp, uv)                   textureGather(sampler2D(COMB(tex, samp)), uv, 0)
   #define _SAMPLE_GATHER_GREEN(tex, samp, uv)                 textureGather(sampler2D(COMB(tex, samp)), uv, 1)
   #define _SAMPLE_GATHER_BLUE(tex, samp, uv)                  textureGather(sampler2D(COMB(tex, samp)), uv, 2)
   #define _SAMPLE_GATHER_ALPHA(tex, samp, uv)                 textureGather(sampler2D(COMB(tex, samp)), uv, 3)
   ...
   #define _LOAD_2D_FLOAT(tex, xy)                             texelFetch(sampler2D(COMB(tex, PS_SAMPLERS[PS_SMP_WRAP_POINT])), int2(xy), 0)
...
//#define VID_USE_INVERTED_Z
   ...
   #define fp16_t4 float16_t4
   #define fp16_t3 float16_t3
   #define fp16_t2 float16_t2
   #define fp16_t  float16_t
   
   #define i16_t4 int16_t4
   #define i16_t3 int16_t3
   #define i16_t2 int16_t2
   ...
   #ifdef _API_VULKAN
      // if dxc sees usage of half it emits 'OpCapability Float16' in SPIR-V code which requires VkPhysicalDeviceFloat16Int8FeaturesKHR.
      // VkPhysicalDeviceFloat16Int8FeaturesKHR is optional in our engine and depends on gpu capabilities.
      // So use half only when we actually need it and know gpu capabilities
      #define half float
      #define half2 float2
      #define half3 float3
      #define half4 float4
