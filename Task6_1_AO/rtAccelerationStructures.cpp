...
void rtAccelerationStructures::OnInit(ID3D12Device5* device, ID3D12GraphicsCommandList5* commandList)
{
   ...
   /*
   {
      hr = dxrStateObject.As(&stateObjectProperties);
      assert(hr == S_OK);

      auto rayGenId = stateObjectProperties.Get()->GetShaderIdentifier(c_raygenShaderName);
      auto missShaderId = stateObjectProperties.Get()->GetShaderIdentifier(c_missShaderName);
      auto hitShaderId = stateObjectProperties.Get()->GetShaderIdentifier(c_hitGroupName);

      UINT shaderIdentifierSize = D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES;

      rayGenCb.viewport = { -1, -1, 1, 1 };
      rootArguments.cb = rayGenCb;


      UINT numShaderRecords = 1;
      UINT shaderRecordSize = AlignSize(shaderIdentifierSize + sizeof(rootArguments), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);

      rayGenShaderTable.Allocate(device, shaderRecordSize * numShaderRecords, L"RayGen Shader Table");
      rayGenShaderTable.MapCpuWriteOnly();
      rayGenShaderTable.shaderRecords.push_back({ {rayGenId, shaderIdentifierSize}, {&rootArguments, sizeof(rootArguments)} });
      m_rayGenShaderTable = rayGenShaderTable.m_resource;

      shaderRecordSize = AlignSize(shaderIdentifierSize, D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);

      missShaderTable.Allocate(device, shaderRecordSize* numShaderRecords, L"Miss Shader Table");
      missShaderTable.MapCpuWriteOnly();
      missShaderTable.shaderRecords.push_back({ {missShaderId, shaderIdentifierSize} });
      m_missShaderTable = missShaderTable.m_resource;

      hitGroupShaderTable.Allocate(device, shaderRecordSize* numShaderRecords, L"HitGroup Shader Table");
      hitGroupShaderTable.MapCpuWriteOnly();
      hitGroupShaderTable.shaderRecords.push_back({ {hitShaderId, shaderIdentifierSize} });
      m_hitGroupShaderTable = hitGroupShaderTable.m_resource;
   }
   */
}
